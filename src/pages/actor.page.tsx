...
import { GET_ALL_ACTOR } from "../api/querys.constant.ts";
...
function ActorPage() {
  ...
  const [typeActors, setTypeActors] = useState<any[]>([]);
  ...
  const fieldsToExclude = ["tipo actor"];
  ...
  const fetchTypeActors = async () => {
    try {
      const result = await fetchEntityData("tipoactor"); // Replace with actual API call
      setTypeActors(result);
    } catch (error) {
