...
import {
  ...
  GET_ALL_VARIABLES_INDICATOR,
} from "../api/querys.constant.ts";

function IndicatorVariablesPage() {
  ...
  const primaryKey = "id";
  const entity = "variablesporindicador";
  const modalFields = [
    "dato",
    "fechadato",
    "fkidindicador",
  ...
  const fieldsToExclude = ["nombre indicador", "id", "nombre variable"];
  ...
  // Fetch variables for the dropdown
  const fetchVariables = async () => {
    try {
      const result = await fetchEntityData("variable"); // Replace with actual API call
      setVariables(result);
    } catch (error) {
...
export default IndicatorVariablesPage;
